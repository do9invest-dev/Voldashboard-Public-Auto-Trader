...
class StreamlitUI:
    ...
    def _render_volatility(self, account):
        ...
        # Get symbols and weights from current holdings
        symbols = []
        weights = []
        ...
        if not symbols:
            st.info("No equity positions to analyze.")
            return
        ...
        # Cache key to avoid re-downloading on every rerun
        cache_key = f"vol_data_{period_label}_{'_'.join(sorted(symbols))}"
        ...
        if st.button("📊 Calculate Volatility", use_container_width=True) or st.session_state[cache_key] is not None:
            try:
                if st.session_state[cache_key] is None:
                    with st.spinner(f"Downloading {period_label} price history for {len(symbols)} holdings..."):
                        # Download all at once — yfinance batches efficiently
                ...
                # Filter to symbols we have data for and re-weight
                available = [s for s in symbols if s in returns.columns]
                if not available:
                    st.warning("No price data available for current holdings.")
                    return
                ...
                # Add SPY benchmark if not already in holdings
                ...
                fig.update_layout(
                    title=f"Portfolio vs SPY — {period_label}",
